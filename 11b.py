...
left = min(visited, key=lambda p: p[0])[0]
right = max(visited, key=lambda p: p[0])[0]
top = min(visited, key=lambda p: p[1])[1]
bot = max(visited, key=lambda p: p[1])[1]
w = right - left + 1
h = bot - top + 1

grid = [[0] * w for _ in range(h)]

for p, col in visited.items():
    x, y = p
    grid[y-top][x-left] = col

# used cartesian coordinates for robot direction ((0, 1) is up) but "normal"
# screen coordinates for the grid, so lines are upside down unless you iterate
# over the lines in reversed order
print('\n'.join(''.join('#' if c == 1 else '.' for c in line) for line in reversed(grid)))
